...
import { 
    ...
    History, AlertTriangle, X, Users, User, Plus, Trash2, Check, RotateCcw, ArrowRight, Briefcase, MessageCircle
} from 'lucide-react';
...
// --- Helper Components (Defined OUTSIDE to fix focus loss) ---
...
export const LeadDetail: React.FC = () => {
  ...
  // Data State
  const [lead, setLead] = useState<Lead | null>(null); // Committed state
  const [localLead, setLocalLead] = useState<Lead | null>(null); // Draft state
  ...
  useEffect(() => {
    ...
    const l = getLead(id);
    if (!l) {
        // If lead not found (or filtered out by RBAC), redirect
        navigate('/'); 
        return;
    }
    setLead(l);
  ...
  }, [id, navigate, currentUser]); // Re-run when user changes to validate access
  ...
  const handleAddBorrower = () => {
    ...
    // Immediate save for structural changes to simplify diffing logic
  ...
  const handleSaveTouch = (e: React.FormEvent) => {
    ...
    // Also update the UI's view of lead stats immediately
  ...
  // --- Change Detection & Modal Logic ---

  const prepareReview = () => {
      if (!lead || !localLead) return;
      const changes: DetectedChange[] = [];

      // 1. Check Root Fields (Status & Assignment)
      ...
      if (lead.assignedTo !== localLead.assignedTo) {
          changes.push({
              ...
              field: 'assignedTo',
              label: 'LO Assignment',
              oldValue: availableUsers.find(u => u.id === lead.assignedTo)?.name || 'Unknown',
              newValue: availableUsers.find(u => u.id === localLead.assignedTo)?.name || 'Unknown',
              reason: 'OTHER',
              comment: 'Reassigned LO',
              isApplied: true
          });
      }
      if (lead.processorId !== localLead.processorId) {
          changes.push({
              ...
              oldValue: availableUsers.find(u => u.id === lead.processorId)?.name || 'Unassigned',
              newValue: availableUsers.find(u => u.id === localLead.processorId)?.name || 'Unassigned',
              reason: 'OTHER',
              comment: 'Updated Processor',
              isApplied: true
          });
      }
      ...
      Object.keys(lead.loanParams).forEach((key) => {
          ...
          if (lead.loanParams[k] != localLead.loanParams[k]) {
              changes.push({
                  id: `lp-${key}`,
                  field: key,
                  label: `Loan ${key.replace(/([A-Z])/g, ' $1').trim()}`, // Camel to Space
                  oldValue: lead.loanParams[k],
                  newValue: localLead.loanParams[k],
                  reason: 'CORRECTION',
                  comment: '',
                  isApplied: true
              });
      ...
      localLead.borrowers.forEach((b, idx) => {
          ...
          if (!original) return; // Should be handled by structural add, but safety check
  ...
  const executeBatchSave = () => {
      ...
      // We start with a copy of localLead (the draft)
      ...
      // If user unchecked some changes, we need to revert those specifically in finalLead
      unappliedChanges.forEach(c => {
          // Revert logic: put oldValue BACK into finalLead
          if (c.id === 'status') {
              finalLead.status = c.oldValue;
          } else if (c.id === 'assignedTo') {
              finalLead.assignedTo = lead.assignedTo;
          } else if (c.id === 'processorId') {
              finalLead.processorId = lead.processorId;
              ...
              finalLead.loanParams[key] = c.oldValue;
               ...
               // Parse ID to get borrower ID and field: b-{id}-{field}
               ...
               if (bIndex >= 0) {
                   // @ts-ignore
                   finalLead.borrowers[bIndex][field] = c.oldValue;
               }
          }
      });

      // Generate Logs for APPLIED changes
  ...
  return (
    <div className="h-full flex flex-col bg-slate-50 relative">
      ...
      <div className="bg-white border-b border-slate-200 px-6 py-4 flex justify-between items-center shadow-sm shrink-0">
        ...
        <div className="flex gap-3 items-center">
             ...
             <div className="flex gap-2 mr-4 border-r border-slate-200 pr-4">
                 ...
                 {/* Processor Assignment - Visible to Admin and LO */}
             ...
             <div className="flex flex-col">
                ...
                <select 
                    ...
                    value={localLead.status} // Edit local status
      ...
      <div className="flex-1 flex overflow-hidden pb-16"> 
        ...
        <div className="w-80 border-r border-slate-200 bg-white overflow-y-auto p-6 space-y-6">
            ...
            <div>
                <div className="flex items-center justify-between mb-3">
                    ...
                    <div className="flex gap-1 items-center">
                        {localLead.borrowers.map((b, idx) => (
                            <button 
                                key={b.id}
                                onClick={() => setActiveBorrowerIndex(idx)}
                                className={`w-6 h-6 rounded flex items-center justify-center text-[10px] font-bold transition-all ${activeBorrowerIndex === idx ? 'bg-brand-600 text-white shadow-md' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'}`}
                        ...
                        {localLead.borrowers.length < 4 && (
                            <button 
                                ...
                                className="w-6 h-6 rounded flex items-center justify-center text-slate-400 hover:text-brand-600 hover:bg-brand-50 transition-colors"
                ...
                <div className="bg-slate-50 rounded-lg p-3 border border-slate-100 transition-all relative">
                    {!activeBorrower.isPrimary && (
                        <button 
                            onClick={() => handleRemoveBorrower(activeBorrowerIndex)}
                            className="absolute top-3 right-3 text-slate-300 hover:text-red-500 transition-colors"
                            title="Remove this co-borrower"
                        >
                            <Trash2 size={14} />
                        </button>
                    )}
                    ...
                    <div className="flex gap-2 mb-3">
                        <SmartInput 
                            value={activeBorrower.firstName} 
                            onChange={(v) => handleUpdateField('firstName', v, 'borrower', activeBorrowerIndex)} 
                            className="w-full" placeholder="First" 
                        />
                        <SmartInput 
                            value={activeBorrower.lastName} 
                            onChange={(v) => handleUpdateField('lastName', v, 'borrower', activeBorrowerIndex)} 
                            className="w-full" placeholder="Last" 
                        />
                    </div>
                    ...
                    <div className="space-y-3">
                        <div className="flex items-center gap-3 text-slate-700">
                            ...
                            <SmartInput 
                                value={activeBorrower.phone} 
                                onChange={(v) => handleUpdateField('phone', v, 'borrower', activeBorrowerIndex)} 
                                placeholder="Phone" 
                            />
                        </div>
                        <div className="flex items-center gap-3 text-slate-700">
                            ...
                            <SmartInput 
                                value={activeBorrower.email} 
                                onChange={(v) => handleUpdateField('email', v, 'borrower', activeBorrowerIndex)} 
                                placeholder="Email" 
                            />
                        </div>
                    </div>
            ...
            <div className="border-t border-slate-100 pt-6">
                ...
                <div className="space-y-4">
                    <SmartSelect 
                        label="Purpose" 
                        value={localLead.loanParams.purpose}
                        onChange={(v) => handleUpdateField('purpose', v, 'loanParams')}
                        options={[{value: 'PURCHASE', label: 'Purchase'}, {value: 'REFINANCE', label: 'Refinance'}, {value: 'HELOC', label: 'HELOC'}]}
                    />
                    
                    <div className="grid grid-cols-2 gap-3">
                        <SmartInput label="Amount ($)" type="number" 
                            value={localLead.loanParams.loanAmount}
                            onChange={(v) => handleUpdateField('loanAmount', parseFloat(v), 'loanParams')}
                        />
                        <SmartInput label="Value/Price ($)" type="number" 
                            value={localLead.loanParams.purchasePrice}
                            onChange={(v) => handleUpdateField('purchasePrice', parseFloat(v), 'loanParams')}
                        />
                    </div>

                    <div className="grid grid-cols-2 gap-3">
                        <SmartInput label="Rate (%)" type="number" 
                            value={localLead.loanParams.interestRate}
                            onChange={(v) => handleUpdateField('interestRate', parseFloat(v), 'loanParams')}
                        />
                        <SmartInput label="FICO" type="number" 
                            value={localLead.loanParams.creditScore || ''}
                            onChange={(v) => handleUpdateField('creditScore', parseFloat(v), 'loanParams')}
                             placeholder="e.g. 740" 
                        />
                    </div>
                </div>
            ...
            <div className="border-t border-slate-100 pt-6">
                ...
                <div className="space-y-4">
                    <div className="grid grid-cols-2 gap-3">
                         <SmartSelect 
                            label="Type" 
                            value={localLead.loanParams.propertyType || PropertyType.SFH}
                            onChange={(v) => handleUpdateField('propertyType', v, 'loanParams')}
                            options={Object.values(PropertyType).map(v => ({value: v, label: v}))}
                        />
                         <SmartSelect 
                            label="Use" 
                            value={localLead.loanParams.propertyUse || PropertyUse.PRIMARY}
                            onChange={(v) => handleUpdateField('propertyUse', v, 'loanParams')}
                            options={Object.values(PropertyUse).map(v => ({value: v, label: v}))}
                        />
                    </div>

                    <div className="grid grid-cols-3 gap-3">
                         <div className="col-span-2">
                             <SmartInput label="State" 
                                value={localLead.loanParams.state || ''}
                                onChange={(v) => handleUpdateField('state', v, 'loanParams')}
                                placeholder="TX" 
                             />
                         </div>
                         <div>
                             <SmartInput label="Zip" 
                                value={localLead.loanParams.zip || ''}
                                onChange={(v) => handleUpdateField('zip', v, 'loanParams')}
                                placeholder="75001" 
                             />
                         </div>
                    </div>
                ...
                {touches.length === 0 ? (
                    ...
                    touches.map((touch) => (
                        <div key={touch.id} className="flex gap-4 group relative pr-8">
                            ...
                            {currentUser.role === UserRole.ADMIN && (
                                <button 
                                    onClick={() => handleDeleteTouch(touch.id)}
                                    className="absolute right-0 top-0 text-slate-300 hover:text-red-500 transition-colors opacity-0 group-hover:opacity-100 p-1"
                                    title="Delete touch (Admin only)"
                                >
                                    <Trash2 size={14} />
                                </button>
                            )}
                ...
                <form onSubmit={handleSaveTouch} className="max-w-4xl mx-auto">
                    <div className="flex gap-4 mb-3">
                        <div className="flex items-center gap-2 bg-slate-100 p-1 rounded-lg">
                            <button 
                                ...
                                className={`px-3 py-1.5 rounded text-xs font-bold transition-all ${touchType === TouchType.CALL ? 'bg-white shadow text-blue-600' : 'text-slate-500 hover:text-slate-700'}`}
                            >
                                Call
                            </button>
                            <button 
                                ...
                                className={`px-3 py-1.5 rounded text-xs font-bold transition-all ${touchType === TouchType.EMAIL ? 'bg-white shadow text-purple-600' : 'text-slate-500 hover:text-slate-700'}`}
                            >
                                Email
                            </button>
                            <button 
                                ...
                                className={`px-3 py-1.5 rounded text-xs font-bold transition-all ${touchType === TouchType.TEXT ? 'bg-white shadow text-indigo-600' : 'text-slate-500 hover:text-slate-700'}`}
                            >
                                Text
                            </button>
                            <button 
                                type="button" 
                                onClick={() => setTouchType(TouchType.WHATSAPP)}
                                className={`px-3 py-1.5 rounded text-xs font-bold transition-all ${touchType === TouchType.WHATSAPP ? 'bg-white shadow text-green-600' : 'text-slate-500 hover:text-slate-700'}`}
                            >
                                WhatsApp
                            </button>
                            <button 
                                type="button" 
                                onClick={() => setTouchType(TouchType.NOTE)}
                                className={`px-3 py-1.5 rounded text-xs font-bold transition-all ${touchType === TouchType.NOTE ? 'bg-white shadow text-slate-700' : 'text-slate-500 hover:text-slate-700'}`}
                            >
                                Note
                            </button>
                        </div>
                        ...
                        <select 
                            ...
                            className="bg-slate-50 border border-slate-300 text-slate-700 text-xs rounded-lg px-3 py-1 focus:ring-2 focus:ring-brand-500"
                        >
                    ...
                    <div className="relative">
                        <textarea
                            ...
                            placeholder="Log your conversation, notes, or next steps..."
                            className="w-full border border-slate-300 rounded-lg p-3 pr-12 text-sm focus:ring-2 focus:ring-brand-500 focus:border-brand-500 resize-none h-24 shadow-inner"
                        />
                        <button 
                            type="submit"
                            disabled={!touchContent.trim()}
                            className="absolute bottom-3 right-3 p-2 bg-brand-600 text-white rounded-md hover:bg-brand-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                        >
                            <Send size={16} />
                        </button>
                    </div>
        ...
        {/* Right Column: AI & Automation & History */}
            ...
            <div className="p-6 space-y-6 overflow-y-auto flex-1">
                ...
                <div className="bg-indigo-50 rounded-xl p-4 border border-indigo-100">
                     <div className="flex items-center gap-2 mb-3">
                        ...
                        <h3 className="font-bold text-indigo-900 text-sm">Smart Summary</h3>
                    </div>
                    {loadingAi ? (
                        ...
                        <p className="text-xs text-indigo-800 leading-relaxed font-medium">
                            {aiSummary || "Log some activity to generate an AI summary."}
                        </p>
                    )}
                </div>
                             ...
                             {lead.changeLog && lead.changeLog.length > 0 ? lead.changeLog.map(log => (
                                 <div key={log.id} className="relative pl-6">
                                     ...
                                     <div className="text-xs text-slate-500 mb-0.5">
                                         {new Date(log.timestamp).toLocaleDateString()} <span className="text-slate-300">•</span> {log.reason.replace('_', ' ')}
                                     </div>
                                     {log.comment && (
                                        <div className="text-xs text-slate-500 italic mb-1 bg-slate-50 p-1 rounded border border-slate-100">
                                            "{log.comment}"
                                        </div>
                                     )}
                                     <div className="text-xs text-slate-800">
                                         <span className="font-bold">{log.field}:</span> {log.oldValue} <span className="text-slate-400">→</span> <span className="font-bold text-slate-900">{log.newValue}</span>
                                     </div>
                                     <div className="text-[10px] text-slate-400 mt-0.5 flex justify-end">
                                        By: {log.changedBy}
                                     </div>
                                 </div>
                             )) : (
      ...
      {/* Unsaved Changes Floating Footer */}
      {isDirty && (
          <div className="absolute bottom-0 left-0 right-0 bg-slate-900 text-white p-4 shadow-2xl flex items-center justify-between z-30 animate-in slide-in-from-bottom duration-300">
              <div className="flex items-center gap-3">
                  ...
                  <div>
                      ...
                      <p className="text-xs text-slate-400">You have modified this lead. Review changes before saving.</p>
                  </div>
              </div>
              <div className="flex gap-3">
                  ...
                  <button 
                    ...
                    className="px-6 py-2 rounded-lg text-sm font-bold bg-brand-600 hover:bg-brand-500 text-white shadow-lg transition-all flex items-center gap-2"
                  >
              ...
              <div className="bg-white rounded-xl shadow-2xl w-[800px] max-h-[90vh] flex flex-col animate-in fade-in zoom-in duration-200">
                  <div className="p-6 border-b border-slate-100 flex justify-between items-center">
                      <h2 className="text-xl font-bold text-slate-900 flex items-center gap-2">
                          <CheckCircle2 className="text-brand-600" /> Review Changes
                      </h2>
                  ...
                  <div className="flex-1 overflow-y-auto p-6 bg-slate-50/50">
                      {detectedChanges.length === 0 ? (
                          <div className="text-center py-10 text-slate-500">No changes detected.</div>
                      ) : (
                          <div className="space-y-4">
                                  ...
                                  <div key={change.id} className={`bg-white border rounded-lg p-4 transition-all ${change.isApplied ? 'border-brand-200 shadow-sm' : 'border-slate-200 opacity-60 grayscale'}`}>
                                      <div className="flex items-start gap-4">
                                          <div className="pt-1">
                                              <input 
                                                type="checkbox" 
                                                checked={change.isApplied}
                                                onChange={() => {
                                                    const updated = [...detectedChanges];
                                                    updated[idx].isApplied = !updated[idx].isApplied;
                                                    setDetectedChanges(updated);
                                                }}
                                          ...
                                          <div className="flex-1">
                                              <div className="flex justify-between items-start mb-2">
                                                  <div>
                                                      ...
                                                      <div className="text-xs flex items-center gap-2 mt-1">
                                                          <span className="bg-red-50 text-red-700 px-1.5 py-0.5 rounded line-through">{String(change.oldValue)}</span>
                                                          <ArrowRight size={12} className="text-slate-400" />
                                                          <span className="bg-green-50 text-green-700 px-1.5 py-0.5 rounded font-bold">{String(change.newValue)}</span>
                                                      </div>
                                                  </div>
                                                  <select 
                                                    value={change.reason}
                                                    onChange={(e) => {
                                                        const updated = [...detectedChanges];
                                                        updated[idx].reason = e.target.value as ChangeReason;
                                                        setDetectedChanges(updated);
                                                    }}
                                                    className="text-xs border border-slate-200 rounded p-1 bg-slate-50 text-slate-600 focus:ring-1 focus:ring-brand-500"
                                                    disabled={!change.isApplied}
                                                  >
                                                      {(['CORRECTION', 'BORROWER_REQUEST', 'SCENARIO_TEST', 'OTHER'] as ChangeReason[]).map(r => (
                                              ...
                                              <input 
                                                ...
                                                className="w-full text-xs border-b border-slate-200 focus:border-brand-500 focus:ring-0 px-0 py-1 bg-transparent"
                                                disabled={!change.isApplied}
                                              />
                                          </div>
                                      </div>
                  ...
                  <div className="p-6 border-t border-slate-100 bg-white rounded-b-xl flex justify-between items-center">
                      <div className="text-sm text-slate-500">
                          <strong>{detectedChanges.filter(c => c.isApplied).length}</strong> changes will be saved.
                      </div>
                      <div className="flex gap-3">
                          <button onClick={() => setIsReviewModalOpen(false)} className="px-5 py-2.5 rounded-lg border border-slate-300 text-slate-700 font-bold hover:bg-slate-50 transition-colors">
                              Cancel
                          </button>
                          <button 
                            onClick={executeBatchSave}
                            className="px-8 py-2.5 rounded-lg bg-brand-600 text-white font-bold hover:bg-brand-700 shadow-lg transition-colors flex items-center gap-2"
                          >
                              <Save size={18} /> Confirm Updates
                          </button>
                      </div>
                  </div>
              </div>
...
const ChevronRightIcon = () => <ChevronRight size={16} className="text-slate-300" />